...
class ConfigManager:
    ...
    def load(self):
        ...
        with open(self.config_path, "r", encoding="utf-8") as f:
            ...
            while not config["target_path"] or not os.path.exists(config["target_path"]):
                config["target_path"] = input(f"请输入素材文件夹路径(因为未在config文件中检测到设置的路径):")
    ...
    def create_default_config(self):
        ...
        print(f"\n\t未找到Config设置文件.\n默认初始config文件将会被自动创建于该路径 : [{self.config_path}]\n\n ")
...
class ImageInfo:
    ...
    def classify_expand_name(self):
        ...
        for file in self.file_info:
            ...
            try:
                ...
                print(f"正在尝试移动文件 [{file}]...\n\t由 [{file_original_path}] -> [{file_new_path}]")
                # 同时将文件路径更新到file_info中
                self.file_info[file]["Path"] = file_new_path
            except Exception as e:
                print(f"文件移动失败 [{file}] : {e}")
                pass
    ...
    def classify_exif(self):
        ...
        if self.exif_classify_mode:
            print(f"\n当前选择的EXIF分类模式为 [{self.exif_classify_mode}]\n")
        else:
            return
        ...
        for file in self.file_info:
            ...
            if file_name in self.exif_dic:
                ...
                # 用户设置其他模式
                    ...
                    print(f"该模式不支持分类 : [{self.exif_classify_mode}].")
                    raise
